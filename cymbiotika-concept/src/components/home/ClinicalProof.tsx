  ...
  return (
    <FadeIn>
      <section className="grid gap-5 rounded-[2rem] border border-[--brand-gold]/30 bg-white p-6 md:grid-cols-[1.1fr_0.9fr] md:p-8">
        <div>
          <p className="text-eyebrow tracking-[0.1em] text-[--brand-mint]">Proof Snapshot</p>
          <h2 className="mt-2 text-h1 leading-tight text-[#1f3126]">Data-forward. Visual-first.</h2>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            {studies.map((study, i) => (
              <article key={study.name} className={`rounded-2xl border p-4 ${i % 2 === 0 ? "bg-white border-[#4e8b5b]" : "bg-white border-[#ea8c2f]"}`}>
                <p className="text-small text-[#34513a]">{study.name}</p>
        ...
        <div className="rounded-2xl border border-[--brand-teal]/30 bg-white p-5">
          <p className="text-eyebrow tracking-[0.1em] text-[#2b5a55]">Comparison</p>
